<template>
  <el-row :gutter="20" class="top-block">
    ...
    <el-col :xs="24" :sm="24" :lg="24">
      <el-main>
          <el-table :data="tableData" empty-text="目前無資料" stripe header-row-class-name="headerStyle">

            <el-table-column prop="language" label="語系">
            </el-table-column>
            <el-table-column prop="topic" label="內容" >
            </el-table-column>
            <el-table-column label="狀態" >
              <template slot-scope="scope">
                <el-switch @change="changeStatus(scope.row)" :value="scope.row.statusOn"
                  active-color="#13ce66"
      ...
      <el-form :model="form" label-position="top" :rules="rules" ref="dataForm" label-width="80px">
        ...
        <el-row>
          <el-col :xs="24" :sm="24" :md="24">
            <el-form-item label="標題" prop="topc">
...
export default {
  ...
  data() {
    ...
    return {
      ...
      rules: {
        topic: [{validator: checkName, trigger: 'change'}]
      }
  ...
  methods: {
    ...
    changeStatus(row){
      ...
      console.log(row)
      this.request.post('/news', row).then(res => {
          if (res.code === '200') {
            this.showResult('success', this.$t('action.save_success'))
            this.loadData()
          } else {
