            ...
            <el-col :span="24" :xs="24" :sm="24" :md="12">
              <el-row class="mb-20 text-right">
                <el-col :span="24" :xs="24" :sm="24" :md="24">
                  ...
                  <el-button v-if="userInfo.empno =='admin' && !googleSigned" type="primary" @click="connectGoogleDrive" icon="el-icon-cloudy"></el-button>
                </el-col>
              </el-row>
              <el-row>
                <el-col :span="24" :xs="24" :sm="24" :md="24">
                  <el-form-item :label="$t('profile.form.account')" prop="empno">
...
export default {
  ...
  methods: {
    ...
    connectGoogleDrive(){
      window.open('/api/googledrive/signin',"_new")
    },
    checkGoogleDriveStatus() {
      this.request.get("/googledrive/signin-status").then(res => {
        console.log('checkGoogleDriveStatus:', res)
        if (res.code === '200') {
          this.googleSigned = true
        } else {
