        ...
        <el-row>
          <el-col :xs="24" :sm="24" :md="8" class="pr-10">
            <el-form-item :label="$t('page.user_maintain.form.account')" prop="empno">
              <el-input v-model="form.empno" autocomplete="off" @focusout="checkExist" :disabled="!canEdit"></el-input>
            </el-form-item>
          </el-col>
          <el-col :xs="24" :sm="24" :md="8" class="pr-10">
...
export default {
  ...
  methods: {
    ...
    checkExist() {
      this.request.get("/user/account/exists", this.form.empno).then(res => {
        if (res.code === '200') {
          //pass
        } else {
          this.form.empno = ''
          this.showResult('error', '帳號重複，請重新輸入')
        }
      })
    },
    setDialogWidth() {
