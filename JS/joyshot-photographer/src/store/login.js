...
const actions = {
    ...
    async submitLoginForm({commit}, data) {
        ...
        if (result.code === '200') {
            setToken('JS_P_TOKEN', result.data.token)
            setToken('JS_P_ROLE_MENU', JSON.stringify(result.data.userMenus))
...
const state = {
    badgeInfo: 0,
    userInfo: {},
    userMenus: getToken('JS_P_ROLE_MENU'),
    token: getToken('JS_P_TOKEN'),
    userAllowedMenus: []
}
const getters = {}
