...
const rules = computed(() => ({
    empno: [{ required: true, message: t('auth.phoneRequired'), trigger: 'blur' }],
...
const handleLogin = async () => {
    ...
    await formRef.value.validate(async (valid) => {
        ...
        try {
            ...
            if (res.data?.token) {
                authStore.setAuth(res.data.token, res.data.userId, res.data.empno, res.data.username, res.data.role, res.data.avatar);
                try {
                    const providerRes = await api.getProvider(res.data.userId);
...
(__VLS_ctx.t('auth.providerSubtitle'));
...
const __VLS_22 = __VLS_asFunctionalComponent(__VLS_21, new __VLS_21({
    ...
    placeholder: (__VLS_ctx.t('auth.phone')),
    prefixIcon: (__VLS_ctx.User),
    size: "large",
    'aria-label': (__VLS_ctx.t('auth.phone')),
